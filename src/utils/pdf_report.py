...
def build_main_analysis_pdf(result: dict, run_settings: dict) -> bytes:
    """Create a complete, printable main-analysis report from one SAIP pipeline result."""
    ...
    story += [paragraph("SAIP", styles["SAIPTitle"]), paragraph("Stock Analysis Report", styles["SAIPH1"]), paragraph(f"{escape(str(company))} ({escape(str(ticker))})", styles["SAIPH2"]), paragraph("This report consolidates the final decision, every agent output, debate record, data-quality notices, and a legend for unavailable values.", styles["SAIPBody"]), spacer(1, 10)]
    story.append(_table([
        ["Run settings", "Value"],
    ...
    story.append(page_break())
    story.append(paragraph("Agent Reports", styles["SAIPH1"]))
    summary_rows = [["Agent", "Score", "Status", "Model"]]
    for name, report in (result.get("agent_reports") or {}).items():
        if not isinstance(report, dict):
            continue
        score = report.get("score", report.get("moat_score", "N/A"))
        summary_rows.append([name, score, f"ERROR: {report['error']}" if report.get("error") else "OK", report.get("_model_used", "N/A")])
    det_risk = result.get("det_risk") or {}
    if det_risk:
        summary_rows.append(["deterministic_risk", det_risk.get("det_risk_score", "N/A"), "OK", "Python rules"])
    story.append(_table(summary_rows, [100, 55, 205, 110], styles, colors))
    for name, report in (result.get("agent_reports") or {}).items():
        if not isinstance(report, dict):
            continue
        story.append(paragraph(name.replace("_", " ").title(), styles["SAIPH2"]))
        story.append(_paragraph(_json_text(report), styles, "SAIPSmall"))
    if det_risk:
        story.append(paragraph("Deterministic Risk", styles["SAIPH2"]))
        story.append(_paragraph(_json_text(det_risk), styles, "SAIPSmall"))

    story.append(page_break())
    story.append(paragraph("Debate", styles["SAIPH1"]))
    debate = result.get("debate") or {}
    if debate.get("error"):
        story.append(_paragraph(f"Debate error: {debate['error']}", styles))
